...
class MHadits extends CI_Model {
	...
	function searchHaditsBool($words, $words_min = NULL, $imam_id, $limit = null) {
		...
		$this->db->select ( table_use () . '.*, ' . docid() . ' AS docid, length(isi_indonesia) as simple, kitab_all.kitab_indonesia, bab_all.bab_indonesia ', FALSE );
		$this->db->join ( 'kitab_all', table_use () . '.' . field ( 'kitab_imam_id' ) . ' = kitab_all.kitab_imam_id AND ' . table_use () . '.' . field ( 'imam_id' ) . ' = kitab_all.imam_id', "left" );
		$this->db->join ( 'bab_all', table_use () . '.' . field ( 'bab_imam_id' ) . ' = bab_all.bab_imam_id AND ' . table_use () . '.' . field ( 'imam_id' ) . ' = bab_all.imam_id', "left" );
		$this->db->order_by ( 'imam_id,simple', 'ASC' );
	...
	function getAllKitab($imam) {
		...
		$msc = microtime ( true );
		//$query = $this->db->query ( $sql );
		$query = $this->db->get_where('kitab_all', array('imam_id' => imam_id($imam)));
		query_exec_time ( microtime ( true ) - $msc );
		return $query->result ();
	}
